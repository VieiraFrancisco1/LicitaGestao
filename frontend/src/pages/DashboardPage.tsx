    ...
    <div className="page-stack dashboard-page">
      ...
      <section className="dashboard-metrics">
        <Metric
          icon={<Gavel />}
          label="Em andamento"
          value={data?.metrics.activeBids ?? 0}
          detail="participações ativas"
        />
        <Metric
          icon={<CalendarClock />}
          label="Próximas sessões"
          value={data?.metrics.upcomingSessions ?? 0}
          detail="nos próximos 7 dias"
        />
        <Metric
          icon={<BellRing />}
          label="Avisos por e-mail"
          value={data?.metrics.pendingConvocations ?? 0}
          detail="pendentes de leitura"
        />
        <Metric
          icon={<TriangleAlert />}
          label="Sessões críticas"
          value={data?.metrics.criticalDeadlines ?? 0}
          detail="sessões vencidas ou em até 3 dias"
        />
      </section>
      ...
      <section className="dashboard-grid-main">
        <article className="dashboard-panel dashboard-attention">
          ...
          <div className="attention-list">
            {!data?.attention.length && (
              <div className="dashboard-empty">Nenhum item crítico no momento.</div>
            )}
            {data?.attention.map((item) => (
              ...
              >
                ...
                <span className="attention-time">
                  {item.type === 'CONVOCATION' ? 'Nova mensagem' : relative(item.days)}
                </span>
        ...
        <article className="dashboard-panel">
          ...
          <div className="dashboard-compact-list">
            {!data?.upcomingBids.length && (
              <div className="dashboard-empty">Nenhuma sessão futura cadastrada.</div>
            )}
            {data?.upcomingBids.map((bid) => (
              <Link key={bid.id} to={`/participacoes/${bid.id}`}>
                ...
                <span>
                  <strong>
                    {bid.municipality}
                    {bid.noticeNumber ? ` · ${bid.noticeNumber}` : ''}
                  </strong>
                  <small>
                    {bid.companyName} · {bid.platformName || 'Sem plataforma'}
                  </small>
                </span>
        ...
        <article className="dashboard-panel">
          ...
          <div className="status-breakdown">
            {!data?.statusBreakdown.length && (
              <div className="dashboard-empty">Sem participações cadastradas.</div>
            )}
            ...
            {data?.companyCards.map((company) => (
              <button key={company.id} onClick={() => setActiveCompanyId(company.id)}>
                ...
                <span className="company-card-copy">
                  ...
                  <small>
                    {company.activeBids} em andamento · {company.upcomingSessions} próximas sessões
                  </small>
                </span>
                {company.pendingConvocations > 0 && (
                  <em>
                    {company.pendingConvocations} aviso{company.pendingConvocations === 1 ? '' : 's'}
                  </em>
                )}
