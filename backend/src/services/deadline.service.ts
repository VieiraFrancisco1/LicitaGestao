...
function tenderScope(auth: AuthScope): Prisma.TenderWhereInput {
  ...
  if (auth.role === UserRole.EMPRESA) {
    return {
      organizationId,
      bids: { some: { companyId: auth.companyId ?? '00000000-0000-0000-0000-000000000000' } }
    };
  }
  return {
    organizationId,
    bids: { some: { company: { organizationId, staffLinks: { some: { userId: auth.userId } } } } }
  };
}
...
export async function listDeadlineAlerts(auth: AuthScope, query: { horizon: number; pastDays: number }) {
  ...
  const alerts = tenders.flatMap((tender) => {
    ...
    return sessionDays >= -query.pastDays && sessionDays <= query.horizon
      ? [{ ...base, key: `SESSION:${tender.id}:${dateOnly(tender.sessionDate)}`, type: 'SESSION' as const, title: 'Sessão da licitação', date: dateOnly(tender.sessionDate), days: sessionDays, severity: severity(sessionDays) }]
      : [];
  });
...
export async function listTenderDeadlines(auth: AuthScope, tenderId: string) {
  ...
  const readKeys = new Set(reads.map((item) => item.alertKey));
  return items.map((item) => ({ ...item, read: readKeys.has(item.key) }));
}
...
export async function markAllDeadlineReads(auth: AuthScope) {
  ...
  await prisma.$transaction(current.items.map((alert) => prisma.notificationRead.upsert({
...
export async function dismissDeadlineAlert(auth: AuthScope, alertKey: string) {
  ...
  if (!current.items.some((item) => item.key === alertKey)) throw new AppError('Notificação não encontrada', 404);
  return prisma.notificationRead.upsert({
